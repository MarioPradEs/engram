...
// TestScopingSC2_AdminBrowserSeesAllObservations (SC2) asserts that an admin session
// on the Browser tab returns ALL observations across users — not just the admin's own.
// S6 decision reversed on 2026-07-07: admins now have full visibility in the Browser tab
// (matching the Contributors tab); non-admins still see only their own.
//
// The admin under test is mario@vivastudios.com. The seed data has observations owned by
// member@vivastudios.com and admin@vivastudios.com — neither of which is mario — so with
// admin full visibility the Browser response must contain BOTH.
func TestScopingSC2_AdminBrowserSeesAllObservations(t *testing.T) {
	t.Parallel()
	...
	body := rec.Body.String()
	// S6 reversed (2026-07-07): admin mario has full visibility in the Browser and sees
	// observations owned by other users (member@ and admin@), even with none of his own.
	if !strings.Contains(body, "Member observation") {
		t.Errorf("SC2: 'Member observation' MUST appear in admin's Browser — S6 reversed, admin sees all")
	}
	if !strings.Contains(body, "Admin observation") {
		t.Errorf("SC2: 'Admin observation' MUST appear in admin's Browser — S6 reversed, admin sees all")
	}
}
