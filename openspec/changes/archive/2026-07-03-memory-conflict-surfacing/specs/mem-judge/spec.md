# Delta for mem_judge Tool (memory-conflict-surfacing)

## Purpose

Defines the `mem_judge` MCP tool contract, the agent behavior heuristics encoded in
`serverInstructions`, the full conflict-loop integration requirements, and the
documentation requirements (Phase H). Together these ensure agents can surface, judge,
and track memory conflicts end-to-end within Phase 1.
...
The system MUST expose a `mem_judge` tool registered in `ProfileAgent` (always available;
not behind ToolSearch or a deferred profile). Agents need `mem_judge` eagerly because
conflict surfacing is reactive within the same `mem_save` round-trip.
...
On success, the `memory_relations` row identified by `judgment_id` MUST be updated with
the new verdict: `relation`, `reason`, `evidence`, `confidence`, `marked_by_actor`,
`marked_by_kind`, `marked_by_model`, `session_id`, and `updated_at` are written;
`judgment_status` flips to `'judged'`. The updated relation row MUST be returned in the
response.

`mem_judge` MUST NOT mutate the target observation. Recording a `supersedes` verdict does
not retract or soft-delete the target — that is a separate deliberate action by the agent.

Re-judging an already-judged row MUST overwrite the verdict and advance `updated_at`.
This is a deliberate revision, not an error. Phase 2 multi-actor resolution may introduce
per-actor row uniqueness; today's overwrite is acceptable because provenance fields capture
the latest actor and timestamp.

`confidence` outside [0.0, 1.0] MUST be clamped to the range boundary; a warning line
MUST appear in the `result` string describing the clamping.
...
- WHEN `mem_judge` is called with `judgment_id = "rel-abc123def45678"`, `relation = "not_conflict"`,
  `confidence = 0.9`, `reason = "different scope: A is local, B is cloud"`
- THEN the relation row has `judgment_status = 'judged'`, `relation = 'not_conflict'`,
  `confidence = 0.9`, `reason = "different scope: A is local, B is cloud"`
...
- GIVEN a relation row with `judgment_status = 'judged'` and `relation = 'compatible'` and
  an `updated_at` timestamp T1
- WHEN `mem_judge` is called again on the same `judgment_id` with `relation = 'conflicts_with'`
  and a new `confidence` value
...
The MCP `serverInstructions` constant in `internal/mcp/mcp.go` MUST include a
`CONFLICT SURFACING` block appended to the existing instructions. The block documents:

1. How to detect that judgment is needed: check for `judgment_required: true` in the
   `mem_save` JSON envelope, or "CONFLICT REVIEW PENDING" in the `result` string.
2. Iteration pattern: call `mem_judge` once per candidate using `candidates[i].judgment_id`.
   The top-level `judgment_id` is a display convenience for the first candidate only.
3. When to ASK the user (conversationally — never via blocking CLI prompt or dashboard):
   - `confidence < 0.7`, OR
   - the intended `relation` is in `{supersedes, conflicts_with}` AND the saved
     observation's `type` is in `{architecture, policy, decision}`.
...
5. Conversational pattern for raising a conflict: surface it in the agent's next reply as
   a natural observation, for example:
   "I noticed memory #18 ('We use sessions for auth') might conflict with what we just
   saved — want me to mark the new one as superseding it, or are they about different
   scopes?"
...
An agent MUST be able to complete the full conflict surfacing loop within a single session:
save a new observation that triggers candidates, call `mem_judge` per candidate, and then
observe the judgment reflected as annotation lines in subsequent `mem_search` results.
This loop is the primary Phase 1 agent-facing workflow.
...
### Requirement: mem_judge Documentation (Phase H)

The project documentation MUST be updated to cover the new conflict surfacing surfaces
introduced in Phase 1, so that agents and integrators can use them without reading the
source code.

`docs/AGENT-SETUP.md` MUST include:
- `mem_judge` tool entry: params, when it fires (after `judgment_required = true` in
  `mem_save` response), and an example conversational flow for an ambiguous case where
  the agent asks the user before recording a verdict.
- Explicit note on Phase 1 deferrals: cloud sync of `memory_relations`, the
  `engram review` decay command, and pgvector embedding generation are not yet active.

The MCP tools reference (`docs/PLUGINS.md` or equivalent) MUST include:
- `mem_judge` tool entry with the full parameter table
  (`judgment_id`, `relation`, `reason`, `evidence`, `confidence`) and the response shape
  (updated relation row with all provenance fields).
- Updated `mem_save` entry documenting the new response fields:
  `id`, `sync_id`, `candidates[]` (with sub-fields), `judgment_id`, `judgment_status`,
  `judgment_required`.
- Updated `mem_search` entry documenting the annotation format:
  `supersedes: #<id>`, `superseded_by: #<id>`, `conflict: contested by #<id> (pending)`.
- Explicit note that `(<title>)` suffix on annotation lines is deferred to Phase 2.
...
- THEN they find documentation covering: when the tool is triggered, required params,
  and an example conversational flow for the ambiguous-verdict case
...
- THEN they find documentation of the new response fields (`candidates`, `judgment_id`,
  `judgment_required`) and the annotation format for `mem_search` results

---

## Phase 1 Deferral Notes

- Retraction of target on `supersedes` verdict: `mem_judge` records the judgment only;
  mutating or soft-deleting the target observation is a separate deliberate agent action.
  Auto-retract is a Phase 2 consideration.
- Multi-actor disagreement resolution: the schema permits multiple verdicts per pair;
  the resolution algorithm (weighted-by-confidence, latest-wins, user-as-tiebreaker) is
  Phase 2.
- Cloud sync of `memory_relations`: deferred to Phase 2.
- `mem_judge` verdict vocabulary extension: the locked verb set in Phase 1 may be extended
  in Phase 2 (e.g., `partially_supersedes`) without breaking Phase 1 clients.
